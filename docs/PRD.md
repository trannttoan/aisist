...
| Prompt injection via email      | Email bodies are untrusted input to the agent. Mitigated by HITL on every mail-changing action, per-call batch caps, and a system prompt rule to treat email content as data.                                                                           |
