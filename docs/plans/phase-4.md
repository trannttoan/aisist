...
`users.messages.list` returns only `id`/`threadId`. Rendering sender, subject, and date requires one `messages.get?format=metadata&metadataHeaders=From,Subject,Date` per result. Do these with `Promise.all` (bounded concurrency), cap search results (recommend 20, schema max 50), and include `snippet`. Quota is fine: 5 + 20×20 = 405 units against 6,000 units per user per minute. Latency is the concern: each `fetchWithAuth` has a 10 s timeout, and sequential gets would be ~3 s for 20 results.
...
- **Prerequisites**: 2.2 (metadata fetch helper shared with search)
...
- **Description**: Schema: `to` (email), `subject`, `body`, `cc` (optional email), `threadId` (optional, "to draft a reply in an existing conversation"). When `threadId` is set: `GET /users/me/threads/{id}?format=metadata&metadataHeaders=Message-ID,Subject,References`, take the last message's `Message-ID` for `In-Reply-To` and append it to `References`, prefix subject with `Re: ` if absent. **Verify with a real token** whether Gmail attaches the draft to the thread; if it needs more than this, cut `threadId` from the schema and note it in the tool description. `POST /users/me/drafts` `{ message: { raw, threadId? } }`. Returns "Draft saved: \"{subject}\" to {to}. Open Gmail to review and send it." No HITL.
- **Files involved**: `backend/src/tools/gmail.ts`, `backend/src/__tests__/tools/gmail.test.ts`, `backend/src/__tests__/agent.test.ts` (bound names)
- **Prerequisites**: 5.1, 2.4
- **Acceptance criteria**: Tests: exact POST body for a new draft, reply draft fetches the thread and sets headers, 404 thread. On device: "draft a reply to the landlord saying the rent is sent" produces a draft in Gmail, threaded (or the schema no longer offers `threadId`).
- **Estimated scope**: small
